...
## 2026-08-11 22:31 Europe/Moscow
...
- Public backend ownership is separate from authenticated learning ownership: `/api/v1/catalog/words` and `/api/v1/catalog/words/{wordID}` are mounted outside the auth middleware; authenticated `/api/v1/words`, `/api/v1/words/{wordID}`, phrases, progress and lessons remain protected.
...
- OpenAPI now documents public content-only list/detail paths and closed `PublicCatalogPage` / `PublicCatalogWord` schemas.
- OpenAPI contract test now parses the complete YAML document with `gopkg.in/yaml.v3` and fails closed if public paths gain auth/status semantics or public word schema gains personalized SRS fields.
...
- CI #3255 initially failed only in frontend source-contract tests after guest/auth ownership changed.
...
- OpenAPI: `api/openapi.yaml`, `backend/internal/words/openapi_contract_test.go`, `backend/go.mod` for the focused YAML parser dependency. Existing `backend/go.sum` already contained the pinned yaml.v3 checksums.
...
- Write-safety read-back for every changed runtime/test path and repeated verification that `main` did not move during writes.
- GitHub-generated diff for the large `LexigoPremiumApp` replacement was exactly the intended six added lines (one import plus exact-return handoff), with no unrelated rewrite.
- GitHub-generated diff for `api/openapi.yaml` was exactly two additive regions: 124 additions, 0 deletions.
- CI #3280 on intermediate head `47834b0faaeb06f8b11b91bb78f73dbe222a75a0`: frontend lint/typecheck/unit/build/audit succeeded; backend unit/race/security and integration/race succeeded before that run was superseded by later acceptance/OpenAPI commits.
- CI #3283 / run `31528038699` started on product/test head `f0bacc3a564afb418c25a877a503d6b7131d87c6`; change-scope classification succeeded. This run is superseded by the Agent Docs commits and is not final immutable-head evidence.

### Checks failed

- Historical CI #3255 frontend unit/source contracts failed on stale/fragile Dictionary and Word Detail assertions; both were corrected with runtime ownership preserved.
- No unresolved functional failure is currently known. Final immutable-head CI is pending after Agent Harness reconciliation commits.

### Current branch head

- Latest product/test head before Agent Docs reconciliation: `f0bacc3a564afb418c25a877a503d6b7131d87c6`.
- Agent Harness scope reconciliation advanced the branch to `d0f7da720d09ea661dedc04bb60d5240d009306c` before this progress write.
- Resolve the live branch ref again after the remaining Agent Harness write(s); that final SHA is the one eligible for immutable-head CI/Ready review.

### Next action

- Complete `.agents/current/EXECUTION.md` evidence.
- Resolve the final branch head and run/poll immutable-head CI until every required backend/frontend/UI/a11y/PWA/visual/performance/container gate is terminal.
- Classify and fix any failing gate without weakening acceptance contracts.
- Audit PR #476 review threads/status and compare live `main` with the expected base.
