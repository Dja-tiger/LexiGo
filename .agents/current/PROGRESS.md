...
## 2026-07-28 21:44 Europe/Moscow
...
- Patcher v1 was deleted. Regex-anchored v2 is exact-branch scoped, contents-write only, excludes bot events and retains strict one-match, prohibited/required marker and exact two-path guards.
- `pull_request` registration did not execute the re-created branch-contained workflow. The proven `push` trigger was restored before this separate developer-authored commit.
...
- Branch-contained `pull_request` registration after workflow recreation did not start v2; no runtime changes resulted.
