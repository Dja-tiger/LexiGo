...
- After the first successful measurement run, lock `/progress` baseline, request ceiling and JavaScript ceiling to exact immutable-head evidence.
...
- The route budget is based on the exact final immutable-head CI measurement, not an estimate.
...
- route bundle performance gate and exact measurement log
...
- Tightening the budget before exact evidence could create a flaky or unverifiable gate.
