...
- `frontend/e2e/learn-browser-zoom.spec.ts` only to reconcile the single CI #4069 assertion that still encodes three lesson-size columns after runtime correctly moved to four
...
- `frontend/e2e/issue-583-compact-library.spec.ts` only to replace the two exact `/learn` 430px fingerprints proven by UI shard 2 artifact `frontend-playwright-report-ui-2` from Linux CI #4069 / run `32648333357`: light `84e41f0c3f35a564df1ef9a821aee3ab58b842b62b9438788cff15ef478f510a` → `b735c5e48f5aaa4a364d7a7b16b48ef168088b4f4ebc904298dba3aa0b5ba2cf`, dark `cfcedd118c241757efc64efdb8e3215f136cad749e99055a79b71f332846bd53` → `737339c0b6395780f25516be3320c3dd478ef9c49718a87010725f071e838825`; both dimensions remain exactly `430×1575`, and provenance must be updated only for those two `/learn` entries to run `32648333357` / head `c4d52f51f944ba0d29c52e0707425ed2473e0267`
- `frontend/e2e/issue-603-browser-zoom-reflow.spec.ts` additionally permits one exact transcription correction after final CI #4078 exposed it: the reviewed `learn.dark` 720×995 fingerprint copied from #4069 as `b07510edb2246d3effceb174593b8ed66d619bb873a23e957f090e77ba003d4b` must be corrected to the primary #4069 Playwright artifact value `b07510edb2246d3effceb174593b8ed66d619bb873a23e957f090e77ba003d4f`; dimensions and provenance remain unchanged at run `32648333357` / head `c4d52f51f944ba0d29c52e0707425ed2473e0267`
- `.github/workflows/temporary-issue-651-stage4-exact-rewrite.yml` **only as a one-shot, exact-anchor, path-guarded large-file rewrite helper for explicitly authorized files; the completed CI #4069 reconciliation changed the single stale 3→4 assertion plus 17 reviewed `/learn` hash/provenance entries; after CI #4078, one additional helper use may perform only the exact Issue #603 `...003d4b` → `...003d4f` transcription correction above, must fail closed on parent/anchor/path drift, and must be deleted before the next final developer-authored candidate and immutable-head CI, leaving zero workflow diff**
...
- CI #4069 evidence must remain the sole source for the 17 refreshed `/learn` hashes: the five visual-owner families keep their reviewed dimensions, and Issue #583 light/dark remain exactly `430×1575`; no unrelated route baseline may change
- primary #4069 `frontend-playwright-report-visual` artifact must be authoritative for the Issue #603 dark 720×995 fingerprint correction; CI #4078 may only diagnose the copied-baseline mismatch, not supply a new baseline
