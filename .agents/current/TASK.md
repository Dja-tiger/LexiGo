...
- `.github/workflows/temporary-issue-651-stage4-exact-rewrite.yml` **only as a one-shot, exact-anchor, path-guarded large-file rewrite helper for explicitly authorized files; for the CI #4069 reconciliation it may change only the single stale 3→4 assertion plus the 17 reviewed `/learn` hash/provenance entries (15 visual-shard fingerprints plus the two Issue #583 UI-shard fingerprints), must fail closed on anchor/count drift, and must be deleted before the final developer-authored candidate and final immutable-head CI, leaving zero workflow diff**
