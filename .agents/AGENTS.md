...
8. [`AGENTS.issue-261-css-specificity.md`](./AGENTS.issue-261-css-specificity.md) — обязательный computed-cascade audit при удалении или консолидации CSS owners.
9. [`AGENTS.issue-132-openapi-structure.md`](./AGENTS.issue-132-openapi-structure.md) — обязательная структурная проверка OpenAPI после изменений YAML.
10. [`AGENTS.issue-132-pg-array-snapshots.md`](./AGENTS.issue-132-pg-array-snapshots.md) — non-null snapshot semantics для пустых PostgreSQL arrays.
11. [`AGENTS.issue-132-dictionary-input-sync.md`](./AGENTS.issue-132-dictionary-input-sync.md) — controlled-input synchronization без stale initial frame.
12. [`SKILLS.md`](./SKILLS.md) — реестр проверенных skills и воспроизводимых project procedures.
13. [`PROJECT_STATE.md`](./PROJECT_STATE.md) — последнее проверенное состояние проекта, roadmap, validation gaps и update protocol.
14. [`current/TASK.md`](./current/TASK.md) — текущий atomic slice, scope, non-goals и invariants.
15. [`current/PROGRESS.md`](./current/PROGRESS.md) — короткий проверяемый журнал фактов.
16. [`current/EXECUTION.md`](./current/EXECUTION.md) — фактически применённые skills и результаты.
17. [`../docs/agent-harness.md`](../docs/agent-harness.md) — архитектура repository memory и полный delivery loop.
18. [`../README.md`](../README.md) и [`../docs/architecture.md`](../docs/architecture.md) — production entrypoints и системные boundaries.
