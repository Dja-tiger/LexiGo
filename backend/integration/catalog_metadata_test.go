...
func TestCatalogMetadataIsPublicCacheableAndTracksCatalogChanges(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	defer rdb.Close()
	cfg := config.Config{AppEnv: "test", HTTPAddr: ":0", LogLevel: "error", CORSAllowedOrigin: "http://test.local", PostgresDSN: requiredEnv(t, "TEST_POSTGRES_DSN"), Redis: config.Redis{Addr: requiredEnv(t, "TEST_REDIS_ADDR")}, JWTSecret: "integration-test-secret-with-at-least-32-bytes", AccessTokenTTL: 15 * time.Minute, RefreshTokenTTL: 24 * time.Hour}
	...
	read := func(request *http.Request, expectedStatus int) (catalogMetadataPayload, http.Header) {
		...
		if requestErr != nil { t.Fatalf("metadata request: %v", requestErr) }
		defer response.Body.Close()
		if response.StatusCode != expectedStatus { t.Fatalf("metadata status = %d, want %d", response.StatusCode, expectedStatus) }
		var payload catalogMetadataPayload
		if expectedStatus != http.StatusNotModified {
			if err := json.NewDecoder(response.Body).Decode(&payload); err != nil { t.Fatalf("decode metadata: %v", err) }
		}
	...
	if first.Totals.Words != catalog.ExpectedCount || first.Totals.Items != first.Totals.Words+first.Totals.Phrases { t.Fatalf("unexpected totals: %+v", first.Totals) }
	if first.Sources.Mixed != first.Totals.Items || first.CatalogVersion == "" || first.UpdatedAt.IsZero() { t.Fatalf("incomplete metadata: %+v", first) }
	etag := headers.Get("ETag")
	if etag == "" { t.Fatal("ETag is empty") }
	...
	if second.Totals.Items != first.Totals.Items+1 || second.Totals.Phrases != first.Totals.Phrases+1 { t.Fatalf("metadata did not track insert: before=%+v after=%+v", first.Totals, second.Totals) }
	if second.CatalogVersion == first.CatalogVersion || secondHeaders.Get("ETag") == etag { t.Fatal("catalog version and ETag must change") }
	found := false
	for _, topic := range second.Topics { if topic.Topic == "Integration Metadata" && topic.Count == 1 { found = true } }
	if !found { t.Fatalf("new topic missing: %+v", second.Topics) }
}
