...
- `/dictionary`, `/progress`, authenticated `/profile`, Scenario catalog/detail, Home, Learn и Active Lesson используют отдельные client entries; только Phrases пока остаётся в compatibility graph `LexigoPremiumApp`;
