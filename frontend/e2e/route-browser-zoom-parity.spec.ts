...
const ROUTES: readonly RouteZoomContract[] = [
  ...
  // Keep onboarding last: its deterministic resume API replaces the shared
  // quality-gate API routing for the remainder of this browser context.
...
const ZOOM_BASELINES: Record<
  `${RouteZoomKey}.${ExplicitAppearance}`,
  ReviewedZoomBaseline
> = {
...
async function browserZoomWorker(context: BrowserContext): Promise<Worker> {
  const existing = context.serviceWorkers().find((worker) => (
    /^chrome-extension:\/\/[a-z]+\/background\.js$/.test(worker.url())
  ));
...
async function expectKeyboardFocusVisible(
  ...
  expect(
    ...
    `${contract.key}: focus-visible must have a painted indicator`,
  ).toBe(true);
  expect(
    focus.left - focus.extent,
    `${contract.key}: focus ring must not clip on the inline start`,
  ).toBeGreaterThanOrEqual(-1);
  expect(
    focus.right + focus.extent,
    `${contract.key}: focus ring must not clip on the inline end`,
  ).toBeLessThanOrEqual(focus.viewportWidth + 1);
}

async function expectZoomedOwnership(
  page: Page,
  contract: RouteZoomContract,
): Promise<void> {
  ...
  expect(
    ...
    `${contract.key}: document must not overflow horizontally at 200% browser zoom`,
  ...
  expect(
    ...
    `${contract.key}: no partially visible focusable control may clip at 200% browser zoom`,
  ).toEqual([]);
  ...
  if (contract.focused) {
    expect(
      ...
      `${contract.key}: focused route must suppress ordinary RouteChrome at 200% zoom`,
    ).toHaveLength(0);
  } else {
    expect(
      geometry.visibleNavigation,
      `${contract.key}: ordinary route must expose exactly one RouteChrome owner at 200% zoom`,
    ).toHaveLength(1);
...
async function captureZoomEvidence(
  ...
  await testInfo.attach(`browser-zoom-200-${contract.key}-${appearance}.png`, {
    body: screenshot,
  ...
  expect(
    actual,
    `${key}: exact Linux 200% browser-zoom fingerprint must match manually reviewed evidence from CI ${baseline.sourceRun} at ${baseline.sourceHeadSha}`,
  ).toEqual({
...
async function runAppearanceMatrix(
  appearance: ExplicitAppearance,
  testInfo: TestInfo,
): Promise<void> {
  ...
  try {
    ...
    for (const contract of ROUTES) {
      ...
      try {
        ...
        ).cssVisualViewport.zoom).toBeCloseTo(1, 4);
        ...
        const reviewMessage = await captureZoomEvidence(
          page,
          testInfo,
          contract,
          appearance,
  ...
  for (const appearance of ["light", "dark"] as const) {
    test(`all ten canonical routes at true 200% browser zoom — ${appearance}`, async ({}, testInfo) => {
      test.skip(
        ...
        "Issue #601 true browser zoom runs once in authoritative desktop Chromium",
      );
