...
const stylesheets = {
  ...
  systemStates: readFileSync(path.join(appDirectory, "system-states.css"), "utf8"),
...
type ShellSnapshot = Readonly<{
  ...
  asyncStateMatchesResourceStack: boolean;
  switchWidth: string;
...
const cases: ReadonlyArray<Readonly<{ width: number; expected: ExpectedInvariant }>> = [
  {
    ...
    expected: {
      ...
      asyncStateMatchesResourceStack: true,
      switchWidth: "328px",
  ...
  },
  {
    ...
    expected: {
      ...
      viewPaddingTop: "24px",
      resourceStackMatchesMainContent: false,
      asyncStateMatchesResourceStack: false,
      switchWidth: "1140px",
      switchMarginTop: "84px",
      switchMarginRight: "40px",
      switchMarginBottom: "20px",
...
const asyncStateBoundaryCases = [
  { width: 1099, matchesResourceStack: true },
  { width: 1100, matchesResourceStack: false },
] as const;
...
async function readShellSnapshot(page: Page): Promise<ShellSnapshot> {
  return page.evaluate(() => {
    ...
    const asyncStateWidth = element(".lx-async-state").getBoundingClientRect().width;
    ...
    return {
      ...
      asyncStateMatchesResourceStack: Math.abs(asyncStateWidth - resourceStackWidth) < 0.5,
...
test.describe("navigation/mobile-shell computed cascade", () => {
  ...
  for (const boundary of asyncStateBoundaryCases) {
    test(`async-state width preserves the 1099/1100px ownership boundary at ${boundary.width}px`, async ({ page }) => {
      await page.setViewportSize({ width: boundary.width, height: 800 });
      ...
      for (const cascade of cascadeOrders) {
        ...
        expect(snapshot.horizontalOverflow, `${cascade.name} horizontal overflow`).toBe(false);
        expect(
          snapshot.asyncStateMatchesResourceStack,
          `${cascade.name} async-state/resource-stack width relation`,
        ).toBe(boundary.matchesResourceStack);
