...
export function CatalogSearchForm({
  ...
  return (
    <form className="lx-catalog-search" role="search" aria-label={label} onSubmit={submit}>
      <label>
        <span className="lx-visually-hidden">Поиск по английскому слову, переводу или теме</span>
        <input
type="search"
value={value}
...
export function CatalogPagination({
  ...
  return (
    <div className="lx-catalog-pagination-shell">
      <p role="status" aria-live="polite" aria-atomic="true">{busy ? "Загружаем страницу каталога…" : catalogRangeText(info)}</p>
      {info.totalPages > 1 ? (
        <nav className="lx-catalog-pagination" aria-label={label}>
<button className="lx-button ghost" type="button" disabled={busy || !info.hasPrevious} onClick={() => onPageChange(info.page - 1)}>← Предыдущая</button>
<span aria-current="page">Страница {info.page.toLocaleString("ru-RU")} из {info.totalPages.toLocaleString("ru-RU")}</span>
<button className="lx-button ghost" type="button" disabled={busy || !info.hasNext} onClick={() => onPageChange(info.page + 1)}>Следующая →</button>
        </nav>
      ) : null}
    </div>
  );
