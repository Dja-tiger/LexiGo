...
export function LexigoApp() {
  ...
  async function loadDueWords(current: Session): Promise<DueWord[]> {
    try {
  ...
  async function startLesson(activeSession = session) {
    ...
    setError("");
    try {
      const dueWords = await loadDueWords(activeSession);
      setWords(dueWords);
      setCurrentIndex(0);
      setRevealed(false);
      setLessonComplete(dueWords.length === 0);
  ...
  async function submitAuth(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const payload = await requestJSON<Session>(`/api/v1/auth/${authMode}`, {
        method: "POST",
        body: JSON.stringify({
          email,
          password,
  ...
  async function logout() {
    ...
    try {
      ...
      // The local session is already removed; server-side logout is best effort.
    }
  }
  ...
  const cards = [
    ...
    { label: "Прогресс", value: lessonStarted ? `${Math.min(currentIndex + 1, words.length)} / ${words.length}` : "0", hint: "текущий урок" },
    ...
    <main className="shell">
      ...
      {!lessonStarted ? (
        ...
        <section className="lesson lesson-complete">
          ...
          <button className="primary-button" type="button" onClick={() => startLesson()} disabled={busy}>Обновить</button>
        </section>
      ) : currentWord ? (
        <section className="study-card" aria-live="polite">
            ...
            {revealed ? (
              <div className="answer-block">
                ...
                {currentWord.examples.length ? (
                  <ul>
                    {currentWord.examples.slice(0, 2).map((example) => <li key={example}>{example}</li>)}
                  </ul>
                ) : null}
          ...
          <div className="study-actions">
            {!revealed ? (
              <button className="primary-button" type="button" onClick={() => setRevealed(true)}>Показать перевод</button>
            ) : (
              <button className="primary-button" type="button" onClick={nextWord}>
                {currentIndex + 1 === words.length ? "Завершить" : "Следующее слово"}
              </button>
            )}
          </div>
        </section>
