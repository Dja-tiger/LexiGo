  ...
  function renderLibrary() {
    return (
      <>
        ...
        {session && progress ? <section className="lx-summary-panel"><div><span>Освоено слов</span><strong>{progress.masteredWords}</strong><small>{catalogMetadataStatus === "ready" && catalogMetadata ? `из ${catalogMetadata.totals.words.toLocaleString("ru-RU")}` : catalogMetadataStatus === "loading" ? "каталог загружается" : "total недоступен"}</small></div><div><span>Освоено фраз</span><strong>{progress.masteredPhrases}</strong><small>{catalogMetadataStatus === "ready" && catalogMetadata ? `из ${catalogMetadata.totals.phrases.toLocaleString("ru-RU")}` : catalogMetadataStatus === "loading" ? "каталог загружается" : "total недоступен"}</small></div><div><span>Due-слова</span><strong>{progress.dueWords}</strong><small>готовы сейчас</small></div><div><span>Due-фразы</span><strong>{progress.duePhrases}</strong><small>готовы сейчас</small></div></section> : null}
      </>
    );
  }
