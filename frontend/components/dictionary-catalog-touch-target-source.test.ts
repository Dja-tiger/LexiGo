...
const informationArchitecture = readFileSync(new URL("../app/information-architecture.css", import.meta.url), "utf8");
const runtime = readFileSync(new URL("./dictionary-catalog.tsx", import.meta.url), "utf8");
const sharedNavigation = readFileSync(new URL("./catalog-kind-navigation.tsx", import.meta.url), "utf8");
...
describe("Issue #74 Dictionary catalog touch-target ownership", () => {
  ...
  it("targets only live Dictionary catalog controls that remain below the modality contract", () => {
    ...
    expect(sharedNavigation).toContain('className="lx-catalog-kind-navigation"');
    expect(sharedNavigation).toContain("Слова и термины");
    expect(sharedNavigation).toContain("Рабочие фразы");

    for (const selector of [
      ".lx-catalog-kind-navigation button",
  ...
  it("preserves painted 34/38/44 geometry while expanding only transparent block-axis hit ownership", () => {
    ...
    expect(informationArchitecture).toContain(".lx-catalog-kind-navigation button {\n  min-height: 44px;");
    expect(informationArchitecture).toContain("@media (max-width: 640px)");
    expect(informationArchitecture).toContain(".lx-catalog-kind-navigation button {\n    min-height: 48px;");
