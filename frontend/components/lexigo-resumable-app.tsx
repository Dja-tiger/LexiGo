...
async function requestJSON<T>(path: string, init: RequestInit = {}, accessToken?: string): Promise<T> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      // Preserve the status when the response has no JSON body.
    }
...
export function LexigoResumableApp() {
  ...
  function closeLesson(clearActive = false) {
    setItems([]);
    setCurrentIndex(0);
    setRevealed(false);
    ...
    setError("");
    if (clearActive) setActiveLesson(null);
  }
  ...
  return (
    <main className="shell">
      ...
      {session && activeLesson && !lessonStarted ? (
          ...
          <fieldset className="option-group compact-group">
            ...
            <div className="segmented-control">
              {GOAL_OPTIONS.map((goal) => (
                <button key={goal} type="button" className={progress.dailyGoal === goal ? "selected" : ""} disabled={busy} onClick={() => updateDailyGoal(goal)}>{goal}</button>
              ))}
            </div>
          </fieldset>
        </section>
      ...
      {!lessonStarted ? (
        <>
          <section className="setup-panel">
            <div className="section-heading">
              <div><p className="eyebrow">НОВЫЙ УРОК</p><h2>Выберите полезный объём на сейчас</h2></div>
              <p>Новый урок заменит незавершённый. Смешанный режим чередует части речи, а не выдаёт однотипные серии.</p>
            </div>
          ...
          <section className="phrases-panel">
            <div className="section-heading">
              <div><p className="eyebrow">TOP TECHNICAL PHRASES</p><h2>Учите рабочие формулировки целиком</h2></div>
            ...
            <div className="phrase-preview-grid">
              {TECHNICAL_PHRASES.slice(0, 6).map((phrase) => (
                <article className="phrase-preview" key={phrase.id}><span>{phrase.topic}</span><strong>{phrase.prompt}</strong><small>{phrase.answer}</small></article>
              ))}
            </div>
        ...
        <section className="all-items-panel">
          <div className="study-toolbar"><button className="secondary-button" type="button" onClick={() => closeLesson(false)}>← Назад к настройкам</button><span>{items.length} элементов · {sourceLabel(source)}</span></div>
          <div className="all-items-list">
            {items.map((item, index) => (
              <article className="all-item" key={item.id}><div className="all-item-number">{index + 1}</div><div><p className="item-tags">{item.partOfSpeech} · {item.topic || "общая лексика"}</p><h3>{item.prompt}</h3>{item.cloze ? <p className="cloze-line">{item.cloze}</p> : null}<strong>{item.answer}</strong>{item.examples[0] ? <p>{item.examples[0]}</p> : null}</div></article>
            ))}
          </div>
        </section>
      ) : lessonComplete ? (
        <section className="lesson lesson-complete">
          <div><p className="eyebrow">СЕССИЯ ЗАВЕРШЕНА</p><h2>{items.length ? "Результаты сохранены" : "Нет элементов для выбранного раздела"}</h2><p>{items.length ? `Знал: ${knownCount}. Почти: ${almostCount}. Не знал: ${againCount}. ${session && progress ? `Сегодня выполнено ${progress.reviewsToday} из ${progress.dailyGoal}.` : ""}` : "Измените раздел, размер блока или откройте технические фразы."}</p></div>
          <div className="completion-actions"><button className="secondary-button" type="button" onClick={() => closeLesson(false)}>К настройкам</button><button className="primary-button" type="button" disabled={busy} onClick={() => startLesson()}>Следующий блок</button></div>
        </section>
      ) : currentItem ? (
        <section className="study-card" aria-live="polite">
          <div className="study-toolbar">
            ...
            <button className="secondary-button" type="button" onClick={() => closeLesson(false)}>{activeLesson ? "Сохранить и выйти" : "Завершить"}</button>
          </div>
